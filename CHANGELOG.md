...
### Added
- A note can now hold a live table of other notes. Write a fenced `base` block in it, in the syntax Obsidian's Bases feature uses, and the note page draws the notes it selects as a table, each row linking to its note. The definition of something like "active subscription" then lives in the Vault as one thing, instead of being worked out again by whoever asks. The table is worked out each time the note is read, from that note's own Vault and no other, and it is never written into the file: the Markdown on disk is exactly what you wrote, and search, backlinks, the graph and statistics see the block and not the rows. A filter against `now()` or `today()` therefore changes its answer as time passes with no file changing. Hatchdoor supports part of the Bases syntax: filters over tags, folders, file names and frontmatter properties with comparisons, `and`, `or`, `not`, emptiness checks and the current time, plus one table view with its columns, their order and a row limit. Anything beyond that is refused with the reason shown in place of the table, rather than half applied, because a filter only partly understood gives a wrong list that looks right. Rows are sorted by note title. A note may hold several blocks, each drawn where it sits and evaluated on its own, and a `<!-- hatchdoor-query: name -->` comment on the line before a block names it without changing what Obsidian reads. The saved queries in one note scan at most 20,000 notes between them, one saved query shows at most 500 rows, and one note holds at most 10; past any of those it says so instead of quietly stopping short. A date written with a space, `2026-09-18 10:00`, or with a time zone compares correctly against `now()`, and `query_notes` benefits from the same rule: two values that both read as dates or times are compared as moments rather than as text. The rows are also available over HTTP at `GET /api/v1/vaults/{vault_id}/notes/{slug}/saved-queries`. [#275]
- A saved query that cannot be drawn now says exactly why, and the three outcomes can no longer be mistaken for one another. A refused block names the part it could not use, such as `daysUntil()` or the line where its YAML breaks, instead of leaving you to delete things until the table appears. A block that is fine but matches nothing says **No matches.** and that it checked every note, so an empty answer cannot pass for a broken one, and a broken one can never show up as an empty table. Grouping, summaries and view types other than `table` no longer refuse the whole block: they only change how rows are drawn, so Hatchdoor draws the plain table with every row and says under it which instruction it left out. A filter it does not understand still refuses everything, since there is no honest partial answer to it. Problems with the `<!-- hatchdoor-query: name -->` markers now show on the page: a marker with no block after it gets a note where it sits, a name that is not usable gets a note under its table, and two blocks sharing a name both still draw their tables, each noting that the name addresses neither until one is renamed. Clicking a column heading sorts that table on your screen, and only that table; the sort is never written to the note and a reload forgets it. Over HTTP the statuses are now `populated`, `empty`, `refused` (with a `construct` naming the part) and `stopped`, presentation instructions that were set aside come back as `ignored`, and marker problems come back beside the queries as `marker_problems` instead of as per-query `notices`. [#276]
- Agents can now read a saved query's rows as data. `get_note` still returns the note exactly as the file holds it and never anything computed. Beside it, a new `saved_queries` list names each saved query in the note. A new tool, `evaluate_saved_query`, takes a note and a saved query's name and returns the notes it selects as rows: each note's Vault, slug and path, plus one value per column the definition picked. The agent picks a saved query by name and never reads the definition, so it cannot drop a condition by rebuilding it as its own query. That was the risk this closes. The name can be left out when the note holds only one saved query. Without it, a note holding several is refused and the refusal lists the names. A saved query is never picked by its position in the note, so moving blocks around changes nothing. An unknown name, a name two saved queries share, a note with no saved query, a definition Hatchdoor cannot evaluate and one stopped at a limit each come back as their own structured error, never as zero rows. An agent that asks about a broken definition cannot report that nothing matched. A query that really matches nothing answers `empty`, and one held back by its own limit or by the 500-row cap says so. The query always reads its own note's Vault, and the tool takes no scope. [#277]
- Agents can rename a tag across a whole Vault in one operation, `rename_tag`, instead of reading and rewriting every note that carries it. Before this, collapsing `domain/homelab` into `topic/homelab` over a few hundred notes meant walking the whole Vault, since tag search stops at 50 results, and then driving the rewrites through batches of 20. The rename always takes two calls. The first writes nothing and answers with the plan: every note that would change, how many change in their frontmatter and how many in their body text, and a `plan_hash`. The second call sends that hash back and applies the plan, but only if the Vault still produces exactly the same plan; if anything changed in between, it is refused and nothing is written. Renaming a tag also renames every tag nested under it, so `domain` takes `domain/homelab` and `domain/plans` with it, even when no note carries `domain` on its own. Matching ignores case, the way tag search does. Renaming into a tag that already exists is a merge: a note left carrying the target twice keeps it once, and the plan tells you how many notes already carried it, which is the number that separates a rename from a merge. Only the renamed characters change. A one-line `tags: [a, b]` list stays on one line, a block list stays a block list, and key order, quoting and every other byte of the note stay as they were. A hashtag inside a code block or inline code is not a tag, so it is left alone. If any note carries the tag in a way that cannot be changed like that, a list the frontmatter editor would restyle for instance, the plan is refused, names each such note and says why, and nothing is written, because a half-renamed Vault is the thing this exists to prevent. If a write fails partway through, every note already rewritten is put back. A synced Vault records the rename as one commit. Deleting a tag is not part of this. [#242]
...
- An accent was dropped from a note's address rather than folded into it, so correcting a title's spelling moved the note. A slug kept ASCII letters and digits and silently discarded every other character, which made `Gerard Veá` into `gerard-ve` and `Café` into `caf`. Renaming a note to add the accent it should have had all along therefore changed the address anything outside the Vault was holding, a bookmark or a link pasted into a chat, while the links inside the Vault were rewritten correctly and gave no sign of it. Two quieter faults came out of the same line. A title written with a combining accent instead of a precomposed letter produced a different address than the visually identical precomposed spelling, so a note's address depended on which machine created the file. And a title with no ASCII letters at all produced no slug, leaving the indexer to fall back to `untitled`, then `untitled-2` and `untitled-3` in the order it happened to walk the folders, so a Vault written in Cyrillic, Greek or any CJK script had no stable addresses: adding one note could move every other note's. The rule now is that European languages give a plain ASCII address and every other script keeps its own letters. An accent folds to the letter it sits on, so `Veá` and `Vea` are one address. The European letters that no accent-stripping reaches are spelled the way their own languages spell them without the letter available: `ß` becomes `ss`, `ø` becomes `o`, `æ` becomes `ae`, `œ` becomes `oe`, `ł` becomes `l`, `đ` and `ð` become `d`, `þ` becomes `th`. Letters and digits from every other writing system are kept as they are, marks included where the script needs them, and nothing is romanised: `資料` stays `資料` rather than turning into `zi-liao`, which would need a dictionary that cannot tell a Japanese reading from a Chinese one. Punctuation and symbols are still dropped, and a name made only of those still falls back to `untitled`. The cost is that a note whose name carries an accent or a non-Latin letter moves address once, from wrong to right, and holds it from then on. There is no redirect from the old spelling; it returns `note_not_found`, because keeping it alive would make the defect a supported way to address a note. Nothing has to be rebuilt to get there. The first index turn after the upgrade notices that the slug it computes for a note no longer matches the one it cached and replaces just those rows. Heading anchors inside a note follow the same rule, so a link to `[[Note#Café]]` lands where it did before. [#306]

[#242]: https://github.com/BatterWorks/Hatchdoor/issues/242
[#275]: https://github.com/BatterWorks/Hatchdoor/issues/275
[#276]: https://github.com/BatterWorks/Hatchdoor/issues/276
[#277]: https://github.com/BatterWorks/Hatchdoor/issues/277
