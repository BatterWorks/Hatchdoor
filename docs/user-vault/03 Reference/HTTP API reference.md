...
| Method | Path | Purpose |
| --- | --- | --- |
| GET | `/api/v1/vaults/{vault_id}/notes/{slug}` | Full note by slug: content, frontmatter, content hash, and `saved_queries`, the note's saved queries by name (`null` for an unnamed one), never evaluated. `404` if absent. |
