...
pub fn layer_from_frontmatter(properties: &serde_json::Value) -> Result<Option<LayerDecl>, String> {
    // Check if hatchdoor key exists
    let Some(hatchdoor) = properties.get("hatchdoor") else {
        return Ok(None);
    };

    // Check if layer key exists
    let Some(layer_value) = hatchdoor.get("layer") else {
        return Ok(None);
    };

    // If layer is null, treat as absent
    if layer_value.is_null() {
        return Ok(None);
    }

    // At this point, layer is present and non-null. It must be a string.
    let Some(raw) = layer_value.as_str() else {
        let type_name = match layer_value {
        ...
        return Err(format!("hatchdoor.layer must be a string, not {type_name}"));
    };
...
mod tests {
    ...
    fn frontmatter_without_hatchdoor_key_declares_nothing() {
        ...
        // Explicit null value for hatchdoor.layer should also declare nothing
        let null_layer = serde_json::json!({ "hatchdoor": { "layer": null } });
        assert_eq!(layer_from_frontmatter(&null_layer).expect("valid"), None);
    }
    ...
    #[test]
    fn frontmatter_rejects_a_non_string_layer_value() {
        // Number value
        let number_layer = serde_json::json!({ "hatchdoor": { "layer": 123 } });
        let err = layer_from_frontmatter(&number_layer).expect_err("number must be rejected");
        assert!(
            err.contains("must be a string"),
            "error should mention type: {err}"
        );
        assert!(err.contains("number"), "error should name the type: {err}");

        // Boolean value
        let bool_layer = serde_json::json!({ "hatchdoor": { "layer": true } });
        let err = layer_from_frontmatter(&bool_layer).expect_err("boolean must be rejected");
        assert!(
            err.contains("must be a string"),
            "error should mention type: {err}"
        );
        assert!(err.contains("boolean"), "error should name the type: {err}");

        // Array value
        let array_layer = serde_json::json!({ "hatchdoor": { "layer": ["sources"] } });
        let err = layer_from_frontmatter(&array_layer).expect_err("array must be rejected");
        assert!(
            err.contains("must be a string"),
            "error should mention type: {err}"
        );
        ...
        // Object value
        let object_layer = serde_json::json!({ "hatchdoor": { "layer": { "name": "sources" } } });
        let err = layer_from_frontmatter(&object_layer).expect_err("object must be rejected");
        assert!(
            err.contains("must be a string"),
            "error should mention type: {err}"
        );
        assert!(err.contains("object"), "error should name the type: {err}");
    }
}
